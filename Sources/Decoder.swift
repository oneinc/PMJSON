...
extension JSON {
    ...
    /// - Parameter options: Options that controls JSON parsing. Defaults to no options. See `JSONOptions` for details.
    /// - Returns: A `JSONStreamDecoder`.
    public static func decodeStream<Seq: Sequence>(_ scalars: Seq, options: JSONOptions = []) -> JSONStreamDecoder<JSONParser<Seq>> {
    ...
    @available(*, deprecated, message: "Use JSON.decodeStream(_:options:) instead")
    public static func decodeStream<Seq: Sequence>(_ scalars: Seq, strict: Bool) -> JSONStreamDecoder<JSONParser<Seq>> {
